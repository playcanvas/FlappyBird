import { TextureAtlasHandler, Vec4, Vec2 } from 'playcanvas';
...
export const getAtlas = (app) => {
  if(!app) throw new Error('App is required');

  if(!atlas) atlas = new TextureAtlasHandler(app).open(ATLAS_URL, ATLAS_DATA);

  atlas.frames = {}
  Object.entries(atlasOpts.frames).forEach(([key, { rect, pivot, border }]) => {
    atlas.setFrame(key, {
      rect: new Vec4(rect[0], rect[1], rect[2], rect[3]),
      pivot: new Vec2(pivot[0], pivot[1]),
      border: new Vec4(border[0], border[1], border[2], border[3]),
    });
  })
